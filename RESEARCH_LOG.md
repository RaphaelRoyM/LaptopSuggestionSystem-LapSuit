...
5. **AI Assistance (ChatGPT)**
  ...
  AI was used during development as a technical assistant to help with debugging, architecture ideas, and documentation.
...
- Vercel, netlify  was considered because it supports GitHub-based deployments and automatic builds.

- However, deployment configuration still requires ensuring that static files such as the dataset CSV are accessible. i understands vercel cannot deploy express js project. i tried deploying via netlify , then only frontend was visible with no functionality. Then there is a need to switch to another platform.

- Then after this i found an another platform which is RENDER. which support express js. so i created a account in render.

- The process of deployment in RENDER ,from account creation is listed below.
Step-by-Step Deployment of the Project on Render
  *8.1.* Create an Account on Render
    - First, open the official website of Render.
    Steps:
  ...
  *8.2.* Prepare the Project on GitHub
    Before deployment, the project must be uploaded to GitHub.
    Steps:
      - Create a repository in GitHub.
      - Upload the project files:
      - Push the code to the repository using Git.
  ...
  *8.3.* Create a New Web Service on Render
    ...
    - Click New
    - Select Web Service
    Render will ask for the Git provider.
    Select GitHub.

  *8.4.* Select the Repository
    Render will show all repositories from your GitHub account.
    Steps:
  ...
  *8.5.* Configure Web Service Settings
    You must fill some configuration details.
    ...
    These commands tell Render how to install dependencies and start the server.

    Instance Type Select:  Free
    This uses Render’s free hosting plan.
  ...
  *8.7.* Environment Variables
    ...
    Render automatically provides the PORT variable, which is used in the server code:
  ...
  *8.9.* Monitor Deployment Logs
    ...
    If there are errors, they appear here.
  ...
  *8.10.* Access the Deployed Application
    Once deployment completes successfully, Render provides a public URL.
    Example format:
      - https://project-name.onrender.com
    This URL allows anyone on the internet to access the application.
...
9. References
  The following resources influenced the project:
    ...
    3. General Articles on Recommendation Systems
      - Studied to understand different recommendation approaches and determine a suitable method for the project.
